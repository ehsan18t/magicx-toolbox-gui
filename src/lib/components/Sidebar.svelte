      ...
      >
        ...
        >
          ...
          {#if badgeCount > 0 && !sidebarStore.isOpen}
            <span
              class="absolute -top-0.5 -right-0.5 flex h-4 min-w-4 items-center justify-center rounded-full border-2 border-surface {badgeColor} px-0.5 text-[9px] font-bold text-white"
            >
        ...
        {#if badgeCount > 0}
          <span
            class="rounded-full {badgeBgColor} px-2 py-0.5 text-xs font-semibold {badgeTextColor} transition-all duration-200 {sidebarStore.isOpen
