...
fn load_tweaks_from_resources() -> Result<HashMap<String, TweakDefinition>, Error> {
    ...
    if tweaks_dir.exists() {
        for category_file in ["privacy.yaml", "performance.yaml", "ui.yaml", "security.yaml", "services.yaml", "gaming.yaml"] {
...
pub fn get_tweaks_for_version(
    version: &str,
) -> Result<HashMap<String, TweakDefinition>, Error> {
