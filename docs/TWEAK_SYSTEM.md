...
(its Settings, actions, ephemerals; ADR-0007); a **value dump** is driven back verbatim, including an effect an OS upgrade has since scoped out (a captured effect the corpus no longer defines fails the restore and keeps the entry), and every shared claim the tweak holds is released, whatever its current scope. Verify;
...
the machine's `MachineGuid`, and the capturing process's user SID.
...
| [0008](./adr/0008-one-build-line-per-snapshots-folder-downgrades-are-unsupported.md) | One build line per snapshots folder: an older build over a newer folder is unsupported, and its marks are not kept readable by older builds |
