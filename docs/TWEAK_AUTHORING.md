...
`probe` answers _"is the state this action produces currently present?"_: **state-based, never history-based.** The **same** probe is the apply-time did-it-work check and the detect-time present/absent contribution. Probe results are **cached per session** and refreshed after an apply/restore of that tweak (detection never re-spawns a shell per status poll). Apply itself never trusts the cache: it re-probes every probeable action live before deciding what to do with it. An action the target option **runs** whose probe already reads present is **left alone**: it is not run, not journaled, and so a later revert never runs its `undo`. That keeps a revert from forcing off a state the machine had before the tweak touched it (an app never installed, a feature already off), even when the rest of the option still had to be applied. An action the target **omits** whose probe reads present is driven back with its `undo` (or disclosed as Residue when it has none, §12.7). Restore of an option builds the same plan. An action **without** `probe` cannot be checked, so an option that runs it always runs it; give an action a probe whenever its state can be read.
...
**A crash is caught on the next launch.** Before an apply or restore changes anything, it marks the snapshot entry it works from as mid-drive; that mark covers Settings and shared settings, which any later verified apply or restore re-establishes. Actions are tracked one by one instead: each action an apply plans gets a journal row, and each action a rollback or restore undoes or re-runs is marked in flight on the entry until it finishes and verifies. So a crash or power loss at any point leaves a mark behind, and the startup scan turns it into Needs Attention even for a tweak made only of Settings or shared settings. The drive item names no effect, because the scan cannot tell which Settings were reached; an action item names its action.
