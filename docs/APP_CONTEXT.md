...
- Apply is atomic *in intent*: any drive or verify failure undoes the completed actions in reverse and drives the captured state back, skipping any effect that already reads as its captured value (an effect the failed drive never moved, such as a task Windows refuses to toggle, is verified by that read rather than re-driven). A rollback that cannot fully complete surfaces as "Needs Attention" (ADR-0001).
