...
- **A3.4** disables `SeDebugPrivilege` on this process, then spawns a no-op TrustedInstaller batch. If the child still spawns, the privilege was not needed; if opening the TI process fails access-denied, it was.
- **F60** spawns a no-op TrustedInstaller batch and calls `IsProcessInJob` on the child handle before it is reaped.
- **C5** launches the apply's child with a minimal machine-only environment block (`lpEnvironment`), then checks the scheduler COM calls still verified under it.
...
Every probe hook is `#[cfg(feature = "test-build")]`, so a normal build carries none of it and the spawn, transport and privilege paths are byte-identical. No operation or diagnostic is added to the elevated child itself: that stays off-limits. What is still out of reach is the child's own view of itself (its environment as it sees it, a job's limits), which no parent-side observation can supply; the cards answer each review item without needing it.

While a probe card is running, its knob is read by any elevated spawn in the process, so do not apply or restore a tweak from another view until the card finishes. This is the same one-thing-at-a-time rule the batch recorder already needs, and in practice only `block_update_pipeline` spawns an elevated child, which the apply-based cards already hold the lock on.
